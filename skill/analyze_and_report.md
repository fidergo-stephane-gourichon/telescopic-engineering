The "anayze and report" mode of the telescopic engineering skill.

Step 1 — Check for prior work
-----------------------------

There might be a still relevant `.telescopic/report.md`.
...
Use the output as a hint, not a rule: if the files returned look like documentation (`.md`, `README`, etc.), staleness is likely. If they are unrelated build artifacts or source files, the prior analysis is probably still valid. When unsure, ask the user.
